...
(venda_sabor.quantidade_por_sabor/venda_total.quantidade_total) * 100 as PARTICIPACAO
FROM
(SELECT TP.SABOR, YEAR(NF.DATA_VENDA) AS ANO, SUM(INF.QUANTIDADE) AS QUANTIDADE_POR_SABOR
 FROM tabela_de_produtos TP
INNER JOIN itens_notas_fiscais INF ON TP.CODIGO_DO_PRODUTO = INF.CODIGO_DO_PRODUTO
...
GROUP BY YEAR(NF.DATA_VENDA)) AS VENDA_TOTAL
ON venda_sabor.ANO = VENDA_TOTAL.ANO
ORDER BY QUANTIDADE_POR_SABOR DESC;

#PODE ACRESCENTAR O ROUND PRA SO APARECER 2 NUMEROS DPS DA VIRGULA

SELECT venda_sabor.sabor, venda_Sabor.ano, venda_sabor.quantidade_POR_SABOR, 
ROUND((venda_sabor.quantidade_por_sabor/venda_total.quantidade_total) * 100, 2) as PARTICIPACAO
FROM
(SELECT TP.SABOR, YEAR(NF.DATA_VENDA) AS ANO, SUM(INF.QUANTIDADE) AS QUANTIDADE_POR_SABOR
 FROM tabela_de_produtos TP
INNER JOIN itens_notas_fiscais INF ON TP.CODIGO_DO_PRODUTO = INF.CODIGO_DO_PRODUTO
